...
def gen_dockerfile_builder_job(String platform, boolean overwrite=false) {
    ...
    if (common.is_open_ci_env) {
        check_docker_image = "docker manifest inspect $common.docker_repo:$tag"
    } else {
...
"""
                                ...
                                sh """\
aws ecr get-login-password --region eu-west-1 | docker login --username AWS --password-stdin $PRIVATE_CONTAINER_REGISTRY
"""
                            }
                            ...
                            if (platform.startsWith('arm-compilers')) {
                                withCredentials(common.is_open_ci_env ? [] : [aws(credentialsId: 'armclang-readonly-keys')]) {
                                    def bucket = common.is_open_ci_env ? "openci-trustedfirmware-private-$INFRA_ENV" : 'trustedfirmware-private'
                                    sh """\
aws s3 presign --region eu-west-1 s3://$bucket/armclang/ARMCompiler6.21_standalone_linux-x86_64.tar.gz >armc6_url
"""
                                    extra_build_args +=
